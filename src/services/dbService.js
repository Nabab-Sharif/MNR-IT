...
// --- Redux-backed cache for cloud tables -------------------------------
// Cloud reads are served from the Redux store (hydrated from the local
// snapshot cache before first paint) and revalidated in the background, so
// pages render instantly instead of waiting for a network roundtrip.
let _reduxApi = null;
async function reduxApi() {
  if (!_reduxApi) {
    const [{ store }, actions] = await Promise.all([
      import("@/store"),
      import("@/store/dataSlice"),
    ]);
    _reduxApi = { store, ...actions };
  }
  return _reduxApi;
}
function reduxApiSync() {
  return _reduxApi;
}
// Kick off module loading immediately so the sync path is ready early.
reduxApi().catch(() => {});

function cachedRows(table) {
  const api = reduxApiSync();
  return api ? api.store.getState().data?.[table]?.items : undefined;
}
function publishRows(table, rows) {
  const api = reduxApiSync();
  if (api) api.store.dispatch(api.setCollection({ storeName: table, items: rows }));
  try {
    const json = JSON.stringify(rows);
    if (json.length < 1.5 * 1024 * 1024) localStorage.setItem("mnr_snap_" + table, json);
  } catch { /* quota — ignore */ }
  try {
    window.dispatchEvent(new CustomEvent("mnr-data-refreshed", { detail: { storeName: table, rows } }));
  } catch { /* noop */ }
}
function publishRow(table, row) {
  const api = reduxApiSync();
  if (!api || !row) return;
  api.store.dispatch(api.upsertRow({ storeName: table, row }));
  publishRows(table, api.store.getState().data?.[table]?.items || []);
}
function publishRemoved(table, id) {
  const api = reduxApiSync();
  if (!api) return;
  api.store.dispatch(api.removeRow({ storeName: table, id }));
  publishRows(table, api.store.getState().data?.[table]?.items || []);
}

const _cloudInflight = new Map();
async function cloudFetch(table) {
  const { data, error } = await supabase.from(table).select("*").order("created_at", { ascending: true });
  if (error) throw error;
  const rows = data || [];
  await reduxApi();
  publishRows(table, rows);
  return rows;
}
async function cloudGetAll(table) {
  await reduxApi();
  const cached = cachedRows(table);
  const revalidate = () => {
    if (_cloudInflight.has(table)) return _cloudInflight.get(table);
    const p = cloudFetch(table).finally(() => _cloudInflight.delete(table));
    _cloudInflight.set(table, p);
    return p;
  };
...
class DBService {
  ...
  async addNVR(nvr) {
    const newNVR = {
      ...nvr,
      id: `nvr_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      created_at: new Date().toISOString(),
    };
