...
const CCTVCheckList = () => {
  ...
  const [printHeader, setPrintHeader] = useState({
    ...
    signatureLabelFontSize: 8,
    signatureNameFontSize: 7,
    signatureUnderline: true,
    signatureTopLine: true,
  });
  ...
  // Legacy rows were created with `Date.now()` ids, so several NVRs / checklists
  // saved in the same millisecond could share one id — deleting one then wiped
  // the others. Give every duplicate a fresh unique id once, on load.
  const repairDuplicateIds = async (nvrsData: any[], checklistsData: any[]) => {
    let changed = false;
    const seenNvr = new Set<string>();
    for (const n of nvrsData) {
      const key = String(n.id);
      if (seenNvr.has(key)) {
        const newId = `nvr_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
        await dbService.upsertNVR({ ...n, id: newId });
        changed = true;
      } else seenNvr.add(key);
    }
    const seenCl = new Set<string>();
    for (const c of checklistsData) {
      const key = String(c.id);
      if (seenCl.has(key)) {
        const newId = `cl_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
        await dbService.upsertCCTVChecklist({ ...c, id: newId });
        changed = true;
      } else seenCl.add(key);
    }
    return changed;
  ...
  const loadData = async () => {
    let nvrsData = await dbService.getNVRs();
    let checklistsData = await dbService.getCCTVChecklists();
    if (await repairDuplicateIds(nvrsData || [], checklistsData || [])) {
      nvrsData = await dbService.getNVRs();
      checklistsData = await dbService.getCCTVChecklists();
    }
    setNvrs(nvrsData || []);
  ...
  const handleExportCCTV = async () => {
    try {
      // Fall back to on-screen state so the file is never empty even if a
      // read fails or the store is still warming up.
      let nvrsData: any[] = [];
      let camerasData: any[] = [];
      let checklistsData: any[] = [];
      try { nvrsData = (await dbService.getNVRs()) || []; } catch { }
      try { camerasData = (await dbService.getCCTVCameras?.()) || []; } catch { }
      try { checklistsData = (await dbService.getCCTVChecklists()) || []; } catch { }
      if (!nvrsData.length) nvrsData = nvrs || [];
      if (!checklistsData.length) checklistsData = checklists || [];

      if (!nvrsData.length && !camerasData.length && !checklistsData.length) {
        toast({ title: "Nothing to Export", description: "No NVR, camera or checklist data found.", variant: "destructive" });
        return;
      }
      ...
      a.click();
      // Keep the blob URL alive long enough for the browser to finish writing.
      setTimeout(() => { a.remove(); URL.revokeObjectURL(url); }, 4000);
      toast({
        title: "Exported",
        description: `${nvrsData.length} NVR(s), ${checklistsData.length} checklist(s) exported.`,
      });
    } catch {
        ...
        <head>
          ...
          <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .page { padding: 2mm 3mm; page-break-after: always; width: 200mm; min-height: 287mm; max-height: 287mm; overflow: hidden; box-sizing: border-box; }
            ...
            .signature-section { display: flex; justify-content: space-between; position: absolute; bottom: 0; left: 3mm; right: 3mm; }
            .sig-block { text-align: center; flex: 1; padding: 0 6px; }
            .sig-space { height: ${printHeader.signatureLineHeight}px; margin-bottom: 2px; }
            .sig-name {
              font-size: ${printHeader.signatureNameFontSize}px;
              font-weight: bold;
              color: #444;
              margin-top: 1px;
              ${printHeader.signatureTopLine ? "border-top: 1px solid #000;" : ""}
              padding-top: 3px;
              ${printHeader.signatureUnderline ? "text-decoration: underline;" : ""}
            }
  ...
  const handleSaveChecklist = async () => {
    ...
    if (duplicate) {
      toast({
        ...
        description: `A checklist for NVR-${selectedNvr.nvr_number} on ${formatDate(checklistFormData.date)} already exists.`,
        variant: "destructive",
      });
      return;
    }
        ...
        <head>
          ...
          <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .sig-space {
              height: ${printHeader.signatureLineHeight}px;
              margin-bottom: 3px;
            }
            .sig-label {
              font-size: ${printHeader.signatureLabelFontSize}px;
              font-weight: bold;
              ${printHeader.signatureTopLine ? "border-top: 1px solid #000;" : ""}
              padding-top: 3px;
              ${printHeader.signatureUnderline ? "text-decoration: underline;" : ""}
        ...
        <Card className="no-print bg-gradient-to-r from-muted/50 to-muted/30">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              ...
              <div className="space-y-2">
                <Label className="text-xs flex items-center gap-1">
                  <Type className="h-3 w-3" /> Signature Name Size
                </Label>
                <Select
                  value={printHeader.signatureNameFontSize.toString()}
                  onValueChange={(v) => setPrintHeader((p) => ({ ...p, signatureNameFontSize: Number(v) }))}
                >
                  <SelectTrigger className="h-8">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {[5, 6, 7, 8, 9, 10, 11, 12, 14].map(size => (
                      <SelectItem key={size} value={size.toString()}>{size}px</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                <Label className="text-xs flex items-center gap-1">
                  <Type className="h-3 w-3" /> Signature Underline
                </Label>
                <Select
                  value={printHeader.signatureUnderline ? "on" : "off"}
                  onValueChange={(v) => setPrintHeader((p) => ({ ...p, signatureUnderline: v === "on" }))}
                >
                  <SelectTrigger className="h-8">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                <Label className="text-xs flex items-center gap-1">
                  <Columns className="h-3 w-3" /> Signature Top Line
                </Label>
                <Select
                  value={printHeader.signatureTopLine ? "on" : "off"}
                  onValueChange={(v) => setPrintHeader((p) => ({ ...p, signatureTopLine: v === "on" }))}
                >
                  <SelectTrigger className="h-8">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                <Label className="text-xs flex items-center gap-1">
                  <Columns className="h-3 w-3" /> Signature Space
                </Label>
                <Select
                  value={printHeader.signatureLineHeight.toString()}
                  onValueChange={(v) => setPrintHeader((p) => ({ ...p, signatureLineHeight: Number(v) }))}
                >
                  <SelectTrigger className="h-8">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {[10, 14, 18, 20, 24, 30, 36, 44].map(h => (
                      <SelectItem key={h} value={h.toString()}>{h}px</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div className="col-span-full flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
                <p className="text-xs text-muted-foreground">
                  <Merge className="h-3 w-3 inline mr-1" />
                  Drag column borders to resize. Signature settings apply to printed reports.
                </p>
                <Button
                  size="sm"
                  onClick={() => {
