...
import { ArrowLeft, Upload, Scissors, Download, Trash2, Loader2, Plus, Printer, Folder as FolderIcon, FolderPlus, FolderOpen, FolderInput, Calendar as CalendarIcon, X, ChevronLeft, ChevronRight, RotateCcw, AlertCircle, MoreVertical, Pencil, ZoomIn, ZoomOut } from "lucide-react";
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <span className="flex items-center gap-2 min-w-0">
                ...
                {zoneSaveStatus !== "idle" && (
                  <span
                    className={`ml-1 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${zoneSaveStatus === "saving"
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-2 flex-wrap items-center">
              {(() => null)()}
              {(() => {
                const refZone = activeZone > 0 && hasCropSize(zones[activeZone - 1])
                  ? zones[activeZone - 1] as PixelCrop
                  : undefined;
                ...
                return zones.map((z, i) => {
                  ...
                  const edgeErrors = z && refZone ? [
                    Math.abs(z.x - refZone.x) > edgeTolerance ? "left" : null,
                    Math.abs((z.x + z.width) - (refZone.x + refZone.width)) > edgeTolerance ? "right" : null,
                    Math.abs(z.y - refZone.y) > edgeTolerance ? "top" : null,
                    Math.abs((z.y + z.height) - (refZone.y + refZone.height)) > edgeTolerance ? "bottom" : null,
                  ].filter(Boolean) as string[] : [];
                  const matches = i === 0 || (!!z && !!refZone && edgeErrors.length === 0);
                    ...
                    <div key={i} className="flex items-center gap-1">
                      ...
                      {refZone && !matches && z ? (
                        <Button
                          ...
                          title={`Match previous crop ${refW}×${refH}`}
                        >
                          = ref
                        </Button>
                      ) : null}
              ...
              <Button size="sm" variant={fitPage ? "default" : "outline"} onClick={() => setFitPage((f) => !f)}>
                {fitPage ? "Fit Page: On" : "Fit Page: Off"}
              </Button>
              <div className="flex items-center gap-1 rounded-md border px-1 py-0.5">
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setZoom((value) => Math.max(0.5, Number((value - 0.1).toFixed(1))))} disabled={zoom <= 0.5} title="Zoom out">
                  <ZoomOut className="h-4 w-4" />
                </Button>
                <span className="w-12 text-center text-xs font-medium">{Math.round(zoom * 100)}%</span>
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setZoom((value) => Math.min(3, Number((value + 0.1).toFixed(1))))} disabled={zoom >= 3} title="Zoom in">
                  <ZoomIn className="h-4 w-4" />
                </Button>
                {zoom !== 1 && <Button size="sm" variant="ghost" className="h-7 px-2 text-xs" onClick={() => setZoom(1)}>Reset</Button>}
              </div>
              <span className="text-xs text-muted-foreground w-full sm:w-auto sm:ml-2">Drag on the image — auto-saves to active zone.</span>
            </div>
            <div className="max-h-[80vh] overflow-auto rounded-lg border bg-muted/30">
              <div className={`relative ${fitPage ? "max-w-full" : "w-full"}`}>
                <ReactCrop
                  key={activeZone}
                  crop={crops[activeZone]}
                  onChange={(c) => setCrops((prev) => { const n = [...prev]; n[activeZone] = c; return n; })}
                  onComplete={(c) => commitZone(c)}
                  className={fitPage ? "max-w-full" : "!w-full"}
                >
                  <img
                    ref={imgRef}
                    src={pages[0]}
                    ...
                    style={
                      fitPage && zoom === 1
                        ? { maxHeight: "calc(100vh - 260px)", maxWidth: "100%", width: "auto", height: "auto", display: "block" }
                ...
                {imgNatural && zones.map((z, i) => {
                  ...
                  const isCompared = i === activeZone - 1 && !!current && hasCropSize(current);
                  const edgeErrors = isCompared ? [
                    Math.abs(z.x - current!.x) > 2 ? "left" : null,
                    Math.abs((z.x + z.width) - (current!.x + current!.width)) > 2 ? "right" : null,
                    Math.abs(z.y - current!.y) > 2 ? "top" : null,
                    Math.abs((z.y + z.height) - (current!.y + current!.height)) > 2 ? "bottom" : null,
                  ].filter(Boolean) as string[] : [];
                  const matchesPrevious = isCompared && edgeErrors.length === 0;
                  const color = isCompared
                    ? matchesPrevious ? "#10b981" : "#ef4444"
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: `${isCompared ? 3 : 2}px ${isCompared ? "solid" : "dashed"} ${color}`,
                        background: `${color}22`,
                    ...
                    >
                      ...
                      >
                        {isCompared ? `${zoneLabel(i)} ${matchesPrevious ? "✓ Same edges" : `! ${edgeErrors.join(", ")}`}` : `${zoneLabel(i)} ✓`}
                      </span>
                    </div>
                  );
                })}
