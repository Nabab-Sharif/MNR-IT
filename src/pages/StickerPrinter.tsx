  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3 print:hidden">
        <Tabs value={rep} onValueChange={setRep} className="min-w-0 w-full sm:w-auto">
          <TabsList className="flex flex-wrap h-auto w-full justify-start text-xs sm:text-sm">
      ...
      <Card className="mb-6 border-t-4 border-t-indigo-500">
        <CardHeader className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            {buyer.logo ? (
              <img src={buyer.logo} alt={buyer.buyer_name} className="h-12 w-12 sm:h-16 sm:w-16 shrink-0 rounded-lg object-contain bg-white border" />
            ) : (
              <div className="h-12 w-12 sm:h-16 sm:w-16 shrink-0 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center font-bold text-xl sm:text-2xl">
                {buyer.buyer_name.charAt(0).toUpperCase()}
              </div>
            )}
            <div className="min-w-0">
              <CardTitle className="text-lg sm:text-2xl break-words">{buyer.buyer_name}</CardTitle>
        ...
        <CardContent className="pt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2 sm:gap-4">
          ...
          {historyDialog && historyDialog.entries.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2 mb-3 items-center">
              ...
              <div className="flex items-center gap-1 flex-wrap w-full sm:w-auto">
                ...
                <Input type="date" value={historyFrom} onChange={(e) => setHistoryFrom(e.target.value)} className="w-[8.5rem] sm:w-36 text-xs sm:text-sm" />
                <Label className="text-xs whitespace-nowrap">To</Label>
                <Input type="date" value={historyTo} onChange={(e) => setHistoryTo(e.target.value)} className="w-[8.5rem] sm:w-36 text-xs sm:text-sm" />
